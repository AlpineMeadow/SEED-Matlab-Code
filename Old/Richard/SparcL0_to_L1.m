...
ParameterPath = 'C:\Users\DF_Local\FalconSEED\MatlabCode\';
run('C:\Users\DF_Local\FalconSEED\MatlabCode\INC_PARAMS_STPSat6_FalconSEED.m');
...
for f=1:numL0files
    ...
    for i=1:len_PACKET_TYPES
        ...
        for j=1:len_PACKET_PARAMS
            for k=1:len_PACKET_PARAM_ATTRIBS
                ...
                end %End of if statement - if exist(paramname) ~= 1
                
            end %End of for loop -   for k=1:len_PACKET_PARAM_ATTRIBS

            param_good(i,j) = sum(param_matrix(i,j,len_PACKET_PARAM_ATTRIBS));
        end%End of for loop -  for j=1:len_PACKET_PARAMS

    end%End of for loop -  for i=1:len_PACKET_TYPES
    ...
    for t=1:length(PACKET_TYPES)
        ...
        for l=0:nvars-1
            ...
            end %End of if statement -  if strcmp(L0_var_name.(['var',num2str(l)]),['data_packet_type_',num2str(t)])
        end %End of for loop - for l=0:nvars-1
        
    end %End of for loop -  for t=1:length(PACKET_TYPES)
