...
%Lets find the indices of the energy bins.
energyBinBoundaries = 20:5:150;

numBoundaries = length(energyBinBoundaries);

%Set up an array that will hold the beginning and ending indices for each
%of the boundaries.
energyBinIndices = zeros(2, numBoundaries);

%Loop through the energyBinBoundaries and find the indices that correspond
%to the boundaries.
for i = 1 : numBoundaries
    if i == 1 
        indices = find(info.energyBins(:,2) < energyBinBoundaries(1));
        energyBinIndices(1, i) = indices(1);
        energyBinIndices(2, i) = indices(end);
    else
        indices = find(info.energyBins(:, 2) <= energyBinBoundaries(i) & ...
            info.energyBins(:, 2) > energyBinBoundaries(i - 1));
        energyBinIndices(1, i) = indices(1);
        energyBinIndices(2, i) = indices(end);
    end
end
...
%Fix the last index.
energyBinIndices(2, 27) = 904;

%Create a filename for the data to be written to.
fileName = [info.STPSat6RootDir, 'STPSat-6_FluxVsTime', info.startYearStr, ...
    info.startMonthStr, info.startDayOfMonthStr, '-', ...
...
for missionDayIndex = info.startMissionDayNumber : info.endMissionDayNumber
    ...
    %Set up an array that will contain the summed energies for the fluxes.
    dt15Flux = zeros(numEvents, numBoundaries);

    %Loop through the energies of the fluxes and sum them according to the
    %bin boundaries set up above.
    for i = 1 : numBoundaries
         data = CDFData.SEED_Electron_Flux_Dt15_Good(:, ...
             energyBinIndices(1, i) : energyBinIndices(2, i))';

         dt15Flux(:, i) = sum(data);
    end
    
    %Now write the .csv file.
    writematrix(dt15Flux, fileName, 'WriteMode', 'append');
