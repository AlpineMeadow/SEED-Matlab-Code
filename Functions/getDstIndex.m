function DstIndex = getDstIndex(info, startYear, startDayOfYear, startHour,...
...
if startDayOfMonth < Dst(1, 3) & startYear == Dst(1, 1) & startMonth == Dst(1, 2)
    disp(['getDstIndex.m : Starting Day of Month is earlier than ', ...
        num2str(Dst(1, 3)), '.  No data exists in the database.'])
end

if endDayOfMonth > Dst(end, 3) & endYear == Dst(end, 1) & endMonth == Dst(end, 2)
    disp(['getDstIndex.m : Ending Day of Month is later than ', num2str(Dst(end, 3)), ...
        '.  No data exists in the database.'])
end

%Find the starting index.
startIndex = find(Dst(:, 1) == startYear & Dst(:, 2) == startMonth & ...
    Dst(:, 3) == startDayOfMonth);

%Find the ending index.
endIndex = find(Dst(:, 1) == endYear & Dst(:, 2) == endMonth & ...
    Dst(:, 3) == endDayOfMonth);
